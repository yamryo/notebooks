...
class Mountains(tuple):
    '''
    4-tuple of integers (x,y,z,w) representing a quasi-cord.
    4 integers follow the rule below;

    1) just one of them is odd number
    2) x or y is odd
    '''
    ...
    def verify_code(self, code):
        ...
        for v in code:
            ...
            if v < 0: raise ValueError("Not non-negative values.")  
        if sum([v%2 for v in code]) != 1:
            raise ValueError("The number of odd numbers is not 1.")
        elif not (code[2]%2 == 0 and code[3]%2 ==0):
            raise ValueError("z and w must be even.")
        else: 
            return True

    def permutation(self):
        pairs = []
        for point in range(sum(self)):
            if point < x:
            pairs += [point, ]
...
def mountains_random_generator(max=10, verbose=False):
    while True:
        half_max = int(max/2) + (max%2)
        print(im)
        try:
            code = [2*np.random.randint(1, half_max) for k in range(4)]
            code[np.random.randint(2)] += 1
            m = Mountains(tuple(code))
            break
